...
void GreedyMerger::mergeSegments(const Segments &originalSegments,
  ...
  // 1. Semi-sort all the segments by its length in a set L
  std::vector<unsigned int> L = partialSortByLength(originalSegments, N_LENGTH_BINS, mImgSize);

  // 2. Semi-sort all the segments by its orientation in a set P_theta
  ...
  for (unsigned int seg_idx : L) {
    ...
    if (is_seg_merged[seg_idx]) {
      // std::cout << "\t The segment has been already merged, so we're going to skip it" << std::endl;
      continue;
    }
    ...
    while (N > 0) {
      ...
      // to both lines l_1 and l_2 have the same sign, while when the
      // segment is nearly horizontal the condition will be that the
      // distances have different sign, we can easily know this looking
      // the signs generated by one of the endpoints
      ...
      for (i = 0; i < N; i++) {
        // If the segment with index i is within the two lines that define the conic
...
GreedyMerger::getTangentLineEqs(Segment seg, float r) {
  ...
  // be a negative number, and so, we should invert the equation sign if it's negative
  if (l1[0] < 0) {
    // If the value is negative change the sign of the equation
    l1 *= -1;
  }
